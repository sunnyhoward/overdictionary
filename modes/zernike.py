...
def Zernike(n_max, x, y, x0=0, y0=0, truncate_circle = True):
    """
    ...
    U = np.zeros([m_max, n_max, ny, nx])
    dUdx = np.zeros_like(U)
    dUdy = np.zeros_like(U)

    # (0,0)
    U[0,0,:,:] = np.ones_like(xx)
    if truncate_circle==True:
        U[0,0,rr>1] = 0                                 
    dUdx[0,0,:,:] = np.zeros_like(xx)                                   
    dUdy[0,0,:,:] = np.zeros_like(xx)                                    

    # (1,0)
    U[1,0,:,:] = yy                               
    dUdx[1,0,:,:] = np.zeros_like(xx)                                    
    dUdy[1,0,:,:] = np.ones_like(xx)  
    if truncate_circle==True:
        dUdy[1,0,rr>1] = 0     
    
    # (1,1)
    U[1,1,:,:] = xx                               
    dUdx[1,1,:,:] = np.ones_like(xx)       
    if truncate_circle==True:
        dUdx[1,1,rr>1] = 0                                
    dUdy[1,1,:,:] = np.zeros_like(xx)                                     
    ...
    dUdx, dUdy = np.gradient(U, axis=(2,3)) #this might be bad but it wasnt fitting properly before that (maybe edges)?

    U,dUdx,dUdy = keep_zernikes(n_max, U, dUdx, dUdy)
    return(U, dUdx, dUdy)



def keep_zernikes(n_zernike, zer, dzerdx, dzerdy):
    '''
    ...
    for n in range(n_zernike):
        for m in range(n+1):
            true_zer.append(zer[n,m])
            true_dzerdx.append(dzerdx[n,m])
            true_dzerdy.append(dzerdy[n,m])

    true_zer = np.stack(true_zer)
    true_dzerdx = np.stack(true_dzerdx)
    true_dzerdy = np.stack(true_dzerdy)
